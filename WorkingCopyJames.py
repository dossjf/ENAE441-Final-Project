...
if __name__ == "__main__":
    ...
    X = np.arange(state_corrected.shape[0]) + 1
    fig6, axs6 = plt.subplots(3,2)
    fig6.suptitle("Pure vs. Corrected Prediction 3-Sigma Bounds")
    for i in range(3):
        axs6[i,0].plot(X,state_pure[:,6+i],"b")
        axs6[i,0].plot(X,-state_pure[:,6+i],"b")
        axs6[i,0].plot(X,state_corrected[:,6+i],"r")
        axs6[i,0].plot(X,-state_corrected[:,6+i],"r")
        axs6[i,0].grid(True)
    ...
    for i in range(3):
        axs6[i,1].plot(X,state_pure[:,9+i],"b")
        axs6[i,1].plot(X,-state_pure[:,9+i],"b")
        axs6[i,1].plot(X,state_corrected[:,9+i],"r")
        axs6[i,1].plot(X,-state_corrected[:,9+i],"r")
        axs6[i,1].grid(True)
        if i == 0:
    ...
    for i in range(3):
        axs7[i,0].plot(X, state_pure[:,6+i],"b")
        axs7[i,0].plot(X,-state_pure[:,6+i],"b")
        axs7[i,0].plot(X, state_pure[:,i]-state_corrected[:,i],"r")
        axs7[i,0].grid(True)
        axs7[i,0].set_ylim([-10000,10000])
        axs7[i,0].set_ylabel(f"{labels[i]} [km]")

    for i in range(3):
        axs7[i,1].plot(X, state_pure[:,9+i],"b")
        axs7[i,1].plot(X, -state_pure[:,9+i],"b")
        axs7[i,1].plot(X, state_pure[:,3+i]-state_corrected[:,3+i],"r")
        axs7[i,1].grid(True)
    ...
    ax8_1.plot(X, state_corrected[:,12],"b")
    ax8_1.set_ylabel("Range Residuals (km)")
    ax8_1.grid(True)
    ax8_2.plot(X, state_corrected[:,13],"b")
    ...
    for i in range(3):
        axs9[i,0].plot(X, state_corrected[:,6+i],"b")
        axs9[i,0].plot(X, -state_corrected[:,6+i],"b")
        axs9[i,0].plot(X, state_corrected[:,i],"r")
        axs9[i,0].grid(True)
        axs9[i,0].set_ylim([-10000,10000])
        axs9[i,0].set_ylabel(f"{labels[i]} [km]")

    for i in range(3):
        axs9[i,1].plot(X, state_corrected[:,9+i],"b")
        axs9[i,1].plot(X, -state_corrected[:,9+i],"b")
        axs9[i,1].plot(X, state_corrected[:,3+i],"r")
        axs9[i,1].grid(True)

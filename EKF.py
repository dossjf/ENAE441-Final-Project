...
def STM(t,Z):
    ...
    drdt = np.zeros(6)
    drdt[0:3] = X[3:6]
    drdt[3:6] = (-mu / r_mag**3) * X[0:3]
    dZdtUF = np.vstack([drdt,dphidt])
    dZdt = dZdtUF.flatten()
    return dZdt
...
def propagate_state(X_in, t_final, t_curr, mu):
    ...
    t, X_out_unprocessed2 = propOrbitTwo(r, v, t_final, t_step, t_curr)
    ...
    F = (np.eye(6) + A*delta_t + (A**2*delta_t**2)/2 + (A**3*delta_t**3)/6) #Unable to get joint STM working, this is a taylor expansion approximation at least.
    End = X_out_unprocessed2[:,-1]
    X_out_new = End[0:6]
    Phi = End[6:].reshape((6,6))

    F = Phi #Comment these lines out to switch to the zero-order hold mode.
    X_out = X_out_new
    return X_out, F
...
def propOrbitTwo(r, v, t_final, t_step, t_curr): #Propagates ECI Coordinates using Numerical Tools
    inputs_OBT = np.concatenate((r, v))
    phi_0 = np.eye(6)
    Z_0 = np.vstack([inputs_OBT,phi_0])
    t_eval = np.arange(t_curr, t_final, t_step)
    sol = solve_ivp(STM, [t_curr, t_final], Z_0.flatten(), t_eval=t_eval, 
                    rtol=1e-12, atol=1e-12) #Change parameter to change propgator tolerance.
    soln = sol.y
    t = sol.t
    return t, soln
...
if __name__ == "__main__":
    ...
    #data = data[:500, :] ######################################################################### Sections some of the data for debugging. Delete later!
    ...
    P_VelVar = 0.010**2 #Velocity Error Variance, Std. Dev - 0.010 km/s.
    P_AccVar = 0.001**2 #Acceleration Error Variance, Std. Dev - 0.01 km/s^2
